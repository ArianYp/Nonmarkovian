"""Train baseline MDLM (masked / absorbing) diffusion: single masked x_t at one timestep,
predict x0 (no router).

MDLM ablation twin of ``train_simple.py``: identical model / data / FBD / checkpointing, with the
forward corruption swapped from Bernoulli simplex flips to absorbing-mask noising and the loss
swapped to the MDLM NELBO (cross-entropy on *masked* positions weighted by the MDLM time factor).

Data: DFM enhancer pickles only (``--dfm_enhancer``). Validation uses the pickle val split.
Optional ``--fbcnn_ckpt`` / ``--fbcnn_num_cls`` / ``--fbcnn_stacks`` for FBD embeddings.
"""
...
from nonmarkovian.forward_mdlm import corrupt_mask, mdlm_loss_weight
...
def _parse_train_simple_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="auto",
        help="DFM enhancer root (the_code/); e.g. data_dfm, auto, or absolute path.",
    )
    p.add_argument(
        "--dfm_melanoma",
        action="store_true",
        help="Melanoma (DeepMEL2) instead of fly brain.",
    )
    p.add_argument(
        "--max_len",
        type=int,
    ...
    p.add_argument("--save", type=str, default="checkpoints/simple_mdlm.pt")
...
def main() -> None:
    ...
    if use_wandb:
        wandb.init(project=args.wandb_project, name=args.wandb_run_name or None, config=cfg)
        wandb.summary["trainer"] = "simple_mdlm"
    ...
    try:
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                if args.log_timing:
                    t0 = tic(device)
                # MDLM NELBO: cross-entropy on MASKED positions only, weighted by the
                # MDLM time factor w(t) (Monte-Carlo over uniform t_start). Replaces the
                # Bernoulli CE-on-all-positions (x T) loss.
                target = x0.clamp(max=3)
                log_probs = F.log_softmax(logits, dim=-1)
                ce = -torch.gather(log_probs, -1, target[:, :, None]).squeeze(-1)  # [B, L]
                t_val = float(t_start + 1) / float(args.num_timesteps)
                w = mdlm_loss_weight(
                    torch.tensor(t_val, device=device),
                    scheduler=args.bernoulli_scheduler,
                )
                nlog_p = w * ce * current_mask.to(dtype=ce.dtype)
                ...
                if use_wandb:
                    ...
                    if args.log_timing:
                        log_payload["train/time_ms_corrupt"] = ms_corrupt
            ...
            if args.log_timing and n_batches > 0 and rank == 0:
                ...
                if use_wandb:
                    wandb.log(
                        {
                            "train/epoch_time_ms_corrupt_avg": sum_ms_corrupt / n_batches,

"""Train baseline discrete diffusion: single noisy x_t at one timestep, predict x0 (no router).

Validation matches ``train.py``: ``--val_split``, ``--val_batch_size``, ``--val_fbd_n``, ``--val_gen_batch``,
``--val_seed``, and optional ``--fbcnn_ckpt`` / ``--fbcnn_num_cls`` / ``--fbcnn_stacks`` for FBD embeddings.
"""
...
from nonmarkovian.validation import compute_fbd_simple, train_val_split, validate_simple
...
def main() -> None:
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Directory with the_code/ (e.g. data_dfm). Use 'auto' to resolve ./data_dfm or repo data_dfm.",
    )
    p.add_argument(
        "--enhancer_split",
        type=str,
        ...
        help="Split inside the DFM pickle",
    )
    p.add_argument(
        "--mel_enhancer",
        action="store_true",
        help="Use DeepMEL2_data.pkl instead of DeepFlyBrain_data.pkl",
    )
    ...
    p.add_argument("--val_seed", type=int, default=0, help="Seed for train/val split (independent of --seed)")
    p.add_argument(
        "--fbcnn_ckpt",
        type=str,
        default="",
    ...
    if args.val_split > 0.0:
        ...
        loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, collate_fn=collate_pad, num_workers=0)
        vb = args.val_batch_size if args.val_batch_size > 0 else args.batch_size
        val_loader = DataLoader(val_ds, batch_size=vb, shuffle=False, collate_fn=collate_pad, num_workers=0)
    else:
        loader = DataLoader(ds, batch_size=args.batch_size, shuffle=True, collate_fn=collate_pad, num_workers=0)
    ...
    if use_wandb:
        ...
        wandb.summary["trainer"] = "simple_discrete"

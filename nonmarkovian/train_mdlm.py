"""Train non-Markovian routed MDLM (masked / absorbing) diffusion on DFM enhancer pickles.

MDLM ablation twin of ``train.py``: identical routed model / router / data / FBD / checkpointing,
with the forward corruption swapped from Bernoulli simplex flips to absorbing-mask noising
(multi-view ``sample_all_views_mask``) and the loss swapped to the MDLM NELBO (cross-entropy on
*masked* positions of the current view, weighted by the MDLM time factor)."""
...
from nonmarkovian.forward_mdlm import mdlm_alpha, mdlm_loss_weight, sample_all_views_mask
...
from nonmarkovian.validation_mdlm import (
    ...
    print_epoch_diffusion_dna_samples_mdlm,
...
def _parse_train_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            "Corruption strategy for multi-view training AND the reverse-sampling constraint "
            "(FBD/preview). 'independent' (non-Markovian): forward views mask i.i.d. per timestep, "
            "and reverse sampling drops the carry-over constraint after --independent_threshold so "
            "the model can revise earlier commitments. 'trajectory' (Markovian): monotone masking "
            "and strict carry-over (once unmasked, stays unmasked)."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=str,
        default="alpha_prev",
        choices=("alpha_prev", "uniform", "mdlm"),
        help=(
            "Per-time weight on the MDLM CE. 'alpha_prev' (default, legacy): scale the batch CE "
            "by the survival prob alpha_{t-1}. 'uniform': no weighting (weight = 1 at every t). "
            "'mdlm': the true MDLM NELBO weight w(t) = alpha'_t / (1 - alpha_t)."
        ),
    )
    p.add_argument(
        "--cond_drop_prob",
    ...
    p.add_argument("--save", type=str, default="checkpoints/routed_mdlm.pt")
    ...
    *,
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if args.log_timing:
                t0 = tic(device)
            # Masked-diffusion multi-view corruption -> 4-channel simplex views (mask -> uniform
            # 1/4) for the router/CNN. Current view is index 0 of the compacted buffer (slots > 0
            # are noisier candidate views).
            views = sample_all_views_mask(
                x0,
                args.num_timesteps,
                t_start=t_start,
            ...
            if args.log_timing:
                t0 = tic(device)
            # Routed (non-Markovian) MDLM loss: the model conditions on the full view history,
            # so every position is predicted through the denoiser -- there is no "copy the
            # unmasked tokens" structure and hence NO mask gating. Weight the per-position CE
            # (over ALL positions) by alpha_{t-1} = survival prob at the previous (less-noisy)
            # step t - 1/T. (The simple/Markovian model keeps the masked-only NELBO weight w(t).)
            target = x0.clamp(max=3)
            log_probs = F.log_softmax(logits, dim=-1)
            ce = -torch.gather(log_probs, -1, target[:, :, None]).squeeze(-1)  # [B, L]
            # Per-time (scalar) weight on the batch CE; see --mdlm_loss_weight.
            if args.mdlm_loss_weight == "uniform":
                loss_weight = torch.tensor(1.0, device=device)
            elif args.mdlm_loss_weight == "mdlm":
                loss_weight = mdlm_loss_weight(
                    torch.tensor(t_cont, device=device),
                    scheduler=args.bernoulli_scheduler,
                )
            else:  # "alpha_prev" (legacy default): survival prob at the previous step t-1
                loss_weight = mdlm_alpha(
                    torch.tensor(t_cont - 1.0 / float(args.num_timesteps), device=device),
                    scheduler=args.bernoulli_scheduler,
                )
            nlog_p = loss_weight * ce

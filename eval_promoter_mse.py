"""Evaluate sp-mse on the promoter val set — identical to SLM's validation_step.

For every val batch: generate sequences with sample_promoter, compute the mean
H3K4me3 SEI profile for real and generated sequences, accumulate MSE.

Usage:
    python eval_promoter_mse.py \
        --ckpt wandb/run-20260506_063200-b1wp1dk3/files/promoter.best.pt \
        --num_batches -1          # -1 = all val batches
        --sampling_steps 128      # default: use value saved in checkpoint
        --batch_size 32           # default: 32 (smaller than training for memory)
        --device cuda:0
"""
...
def _build_model(saved_args: dict) -> RoutedDenoiserPromoter:
    a = saved_args
    return RoutedDenoiserPromoter(
        max_len=a.get("seq_length", 1024),
        num_timesteps=a.get("num_timesteps", 1000),
        embed_dim=a.get("embed_dim", 256),
        n_hidden=a.get("n_hidden", 256),
        router_tau=a.get("router_tau", 1.0),
        router_k=a.get("router_k", 1),
        router_conv_kernel=a.get("router_conv_kernel", 1),
        router_out_channels=a.get("router_out_channels", 128),
        signal_channels=1,
    )
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    print(f"  split={args.split}  sequences: {len(val_ds):,}  "
          f"| batch_size: {batch_size}  "
          f"| evaluating {num_batches}/{len(val_loader)} batches")

    # ── sampling config ───────────────────────────────────────────────────────
    sampling_steps = args.sampling_steps or saved_args.get("sampling_steps", 128)
    num_timesteps  = saved_args.get("num_timesteps", 1000)
    scheduler      = saved_args.get("bernoulli_scheduler", "loglinear")
    history_mode   = saved_args.get("history_mode", "trajectory")
    print(f"  sampling_steps={sampling_steps}  num_timesteps={num_timesteps}  "
          f"scheduler={scheduler}")

    # ── evaluate — identical to SLM validation_step ───────────────────────────
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        # real sequences
        real_oh  = F.one_hot(x0, num_classes=4).float()   # [B, L, 4]
        real_sc  = _get_sei_profile(sei, h3k4me3_mask, real_oh, device)  # [B]

        # generated sequences — same conditioning signal, same procedure as SLM
        ...
        gen_sc   = _get_sei_profile(sei, h3k4me3_mask, gen_oh, device)   # [B]
    ...
    print(f"\nFinal sp-mse: {final:.6f}  (over {len(sp_mse_vals)} batches × {batch_size} sequences)")
    return final
...
def main() -> None:
    ...
    p.add_argument("--batch_size",     type=int, default=32,
                   help="Batch size for evaluation (smaller = less GPU memory).")
